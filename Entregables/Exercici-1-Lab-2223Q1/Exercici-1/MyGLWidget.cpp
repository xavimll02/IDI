...
void MyGLWidget::pintaNino()
{
  ...
  modelTransformBracDret(); // Transformem adequadament el brac dret
  pintaBrac();              // i el pintem
  
  modelTransformBracEsquerre(); // Transformem adequadament el brac esquerre
  pintaBrac();                  // i el pintem
  
  if (estat != 0) {modelTransformCorona(); pintaCorona();}
}  
...
{
  glm::mat4 TG(1.0f);
  // Anem on volem respecte l'eix de coordenades
  TG = glm::translate(TG,glm::vec3(0.15,0.15,0.0)); 
  // Apliquem una rotacio de 120 graus (90 + 30)
  if (estat == 2) TG = glm::rotate(TG,float(M_PI/2+M_PI/6),glm::vec3(0.0,0.0,1.0));
  // Ubiquem el punt de rotacio en el centre de coordenades
  TG = glm::translate(TG,glm::vec3(0.0,-0.2,0.0));
  // A x li hem aplicat un escalat de 0.2 perque hem vist que l'amplada original
  // dels bracos (a creaBuffers) era 0.5 i 0.5*0.2=0.1 d'amplada i y hem vist que tenia 
  //2.0 de llargada, per tant l'escalat en y tambe es de 0.2, per assolir una llargada de 0.4
  TG = glm::scale(TG,glm::vec3(0.2,0.2,0.0)); 
  glUniformMatrix4fv(TGLoc, 1, GL_FALSE, &TG[0][0]); 
}
...
{
  glm::mat4 TG(1.0f);
  // Mateixa explicacio que brac dret, en aquest cas la rotacio sera negativa
  TG = glm::translate(TG,glm::vec3(-0.15,0.15,0.0));
  if (estat == 2) TG = glm::rotate(TG,float(-(M_PI/2+M_PI/6)),glm::vec3(0.0,0.0,1.0));
  TG = glm::translate(TG,glm::vec3(0.0,-0.2,0.0));
  TG = glm::scale(TG,glm::vec3(0.2,0.2,0.0));
...
{
  glm::mat4 TG(1.0f);
  // Despres d'analitzar creaBuffersCapICos hem vist que de (0,0,0) al punt on acaben els
  // cabells hi havia una distancia de 1.25 i que se li aplicava a modelTransformCapICos
  // un escalat de 0.4 i per tant la distancia passava a ser 0.5 i a aixo li hem sumat
  // 0.2 = hcorona/2 perque comenci just dalt els cabells
  TG = glm::translate(TG,glm::vec3(0.0,0.7,0.0));
  // Despres d'analitzar creaBuffersCorona -> amplada = 1.5*0.2=0.3, h = 2*0.2=0.4
  TG = glm::scale(TG,glm::vec3(0.2,0.2,0.0));
  glUniformMatrix4fv(TGLoc, 1, GL_FALSE, &TG[0][0]); 
}
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)
{
  makeCurrent();
  switch (event->key()) {
      case Qt::Key_C: { // escalar a més gran
